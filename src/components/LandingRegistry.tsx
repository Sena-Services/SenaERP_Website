              ...
              {selectedRegistryItem ? (
                ...
                <div className="h-full flex flex-col pl-2">
                  ...
                  <div className="flex-shrink-0 mb-5">
                    <span className="text-[10px] font-semibold uppercase tracking-widest text-sena-blue">
                      ...
                      <div className="space-y-2.5">
                        {selectedRegistryItem.bullets.map((bullet, idx) => (
                          <div key={bullet} className="flex items-baseline gap-3">
                            <span className="text-sena-blue text-sm font-medium" style={{ fontFamily: "Georgia, serif" }}>{String(idx + 1).padStart(2, '0')}</span>
                  ...
                  <div className="flex-shrink-0 pt-4 mt-auto border-t border-gray-200">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[11px] font-medium text-sena-blue tracking-wide">Production Ready</span>
                    </div>
                  </div>
                </div>
              ) : !activeEnvironment ? (
            ...
            return (
              <motion.div
                ...
                className={`rounded-2xl bg-white overflow-hidden transition-all duration-200 ${
                  isSelected
                    ? 'border-2 border-sena-blue shadow-lg'
                ...
                <div className="p-4">
                  <div className="flex items-center gap-3">
                    <div className={`p-2.5 rounded-xl flex-shrink-0 transition-colors ${
                      isSelected ? 'bg-sena-blue text-white' : 'bg-gray-100 text-sena-blue'
                    }`}>
                        ...
                        <div className="mb-4">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {tags.map((tag: string) => (
                              <span
                                key={tag}
                                className="px-2.5 py-1 text-xs font-medium bg-sena-blue/10 text-sena-blue rounded-lg"
                              >
                                {tag}
                              </span>
                            ))}
                          ...
                          <ul className="space-y-2">
                            {item.bullets.map((bullet) => (
                              <li key={bullet} className="flex items-start gap-2 text-sm text-gray-600">
                                <svg className="w-4 h-4 text-sena-blue flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
